...
export default function PricingSection() {
  ...
  const fallbackPricingData = t.pricing || {
    ...
    description:
      "Dari pembuatan faktur, pembelian, pengelolaan inventaris, manajemen aset tetap, hingga laporan keuangan dan analisis bisnis, semuanya tersedia dalam satu platform.",
    plans: []
  };

  // Map API data ke format yang dibutuhkan atau gunakan fallback
  const pricingData =
    pricingPlans.length > 0
      ? {
          title: fallbackPricingData.title,
          subtitle: fallbackPricingData.subtitle,
          description: fallbackPricingData.description,
          plans: pricingPlans.map(plan => ({
            name: plan.displayName,
            price: plan.price === 0 ? "Gratis" : `Rp ${plan.price.toLocaleString("id-ID")}`,
            pricePeriod: plan.billingCycle === "monthly" ? "/bulan" : "/tahun",
            userRange: plan.planName,
            features: [plan.description],
            ctaText: "Hubungi Kami",
            badgeColor: plan.displayOrder === 1 ? "orange" : plan.displayOrder === 2 ? "blue" : "green"
          }))
        }
      : fallbackPricingData;
      ...
      <section className="pt-32 lg:pt-40 pb-12 lg:pb-20 bg-gradient-to-b from-primary-50 via-white to-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-center items-center min-h-[400px]">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-[#0498da]" />
          </div>
        </div>
      </section>
    );
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 lg:mb-16 max-w-4xl mx-auto">
          <ScrollAnimation direction="fade" delay={0} duration={600}>
            ...
            <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-4 lg:mb-5">{pricingData.subtitle}</p>
            <p className="text-sm lg:text-base text-gray-600 leading-relaxed max-w-3xl mx-auto">{pricingData.description}</p>
          </ScrollAnimation>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 max-w-7xl mx-auto">
          {pricingData.plans.map((plan, index) => (
            <ScrollAnimation key={index} direction="up" delay={index * 150} duration={600} className="flex">
              <div className="bg-white rounded-2xl shadow-xl border-2 border-gray-100 hover:shadow-2xl hover:border-primary-400 hover:-translate-y-2 transition-all duration-300 flex flex-col w-full group overflow-hidden relative">
                ...
                <div
                  className={`relative ${getBadgeColorClass(
                    plan.badgeColor
                  )} text-white px-5 lg:px-6 py-4 lg:py-4.5 overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent" />
                  <span className="relative font-bold text-base lg:text-lg uppercase tracking-wider">{plan.name}</span>
                </div>
                ...
                <div className="p-6 lg:p-8 flex flex-col flex-1 relative z-10">
                  ...
                  <div className="mb-5 pb-5 border-b-2 border-gray-100">
                    <div className="flex items-baseline gap-2 mb-2">
                      <div className="text-4xl lg:text-5xl font-bold text-gray-900">{plan.price}</div>
                    </div>
                    <div className="text-xs lg:text-sm text-gray-500 font-medium">{plan.pricePeriod}</div>
                    {/* User Range Badge */}
                    <div className="mt-4 inline-flex items-center px-3 py-1.5 bg-primary-50 rounded-full border border-primary-200">
                      ...
                      <span className="text-xs lg:text-sm font-semibold text-primary-700">{plan.userRange}</span>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <svg className="w-5 h-5 lg:w-6 lg:h-6 relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                ...
                <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-primary-500/10 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
              </div>
            </ScrollAnimation>
          ))}
        </div>
