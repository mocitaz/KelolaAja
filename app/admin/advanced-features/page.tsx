...
export default function AdvancedFeaturesPage() {
  ...
  const [editingFeature, setEditingFeature] = useState<AdvancedFeature | null>(null);
  const [previewFeature, setPreviewFeature] = useState<AdvancedFeature | null>(null);
  ...
  const fetchFeatures = async () => {
    try {
      const response = await apiFetch('/api/v1/advanced-features/admin');
  ...
  const handleDelete = async (featureId: number) => {
    ...
    try {
      const response = await apiFetch(`/api/v1/advanced-features/admin/${featureId}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        fetchFeatures();
  ...
  const filteredFeatures = features.filter(
    (feature) =>
      feature.iconFileId?.toString().includes(searchTerm) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            <RocketLaunchIcon className="w-8 h-8 text-[#039edb]" />
            Advanced Features
          </h1>
          <p className="text-gray-600 mt-2">Manage advanced features displayed on homepage</p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-[#039edb]">
          <div className="flex items-center justify-between">
            ...
            <RocketLaunchIcon className="w-12 h-12 text-[#039edb]/20" />
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search features by title or icon file ID..."
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">
            {searchTerm ? 'No features found' : 'No advanced features yet'}
          </h3>
          <p className="text-gray-600 mb-6">
            {searchTerm
              ? 'Try adjusting your search terms'
              : 'Start by adding your first advanced feature'}
          </p>
          {!searchTerm && (
            <button
              onClick={() => {
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-xs text-gray-500">Icon File ID: {feature.iconFileId}</p>
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full my-8">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center rounded-t-xl">
          <h2 className="text-xl font-bold text-gray-900">
            {feature ? 'Edit Advanced Feature' : 'Add New Advanced Feature'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
              <RocketLaunchIcon className="w-5 h-5 text-[#039edb]" />
              Basic Information
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Icon File ID *</label>
                <input
                  type="number"
                  required
                  min="1"
                  value={formData.iconFileId}
                  onChange={(e) => setFormData({ ...formData, iconFileId: parseInt(e.target.value) })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-[#039edb]"
                  placeholder="5"
                />
                <p className="text-xs text-gray-500 mt-1">Media file ID for the icon</p>
              </div>
...
function PreviewModal({ feature, onClose }: { feature: AdvancedFeature; onClose: () => void }) {
  return (
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full">
        ...
        <div className="p-6 space-y-6">
          <div className="text-center bg-gradient-to-r from-[#039edb]/5 to-[#71bf44]/5 rounded-lg p-8">
            ...
            <p className="text-sm text-gray-500">Icon File ID: {feature.iconFileId}</p>
          </div>
