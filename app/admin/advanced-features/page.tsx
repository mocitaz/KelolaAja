...
import { PlusIcon, PencilIcon, TrashIcon, RocketLaunchIcon } from '@heroicons/react/24/outline';
...
export default function AdvancedFeaturesPage() {
  ...
  const handleDelete = async (featureId: number) => {
    ...
    try {
      await apiFetch(`/api/v1/advanced-features/admin/${featureId}`, {
        method: 'DELETE',
      });
      fetchFeatures();
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      header: 'Icon File ID',
      render: (feature: AdvancedFeature) => (
        <span className="text-xs text-gray-600">{feature.iconFileId || '-'}</span>
      ),
    },
    {
      header: 'Order',
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Manage advanced features displayed on homepage"
        action={{
          label: 'Add Feature',
          onClick: () => {
      ...
      <AdminCard compact>
        <SearchBar
          value={search}
          onChange={setSearch}
          placeholder="Search by icon file ID or title..."
        />
      </AdminCard>
      ...
      <AdminTable
        ...
        loading={loading}
        emptyMessage="No advanced features found. Click 'Add Feature' to create one."
      />
...
function AdvancedFeatureModal({
  ...
  return (
    <AdminModal
      isOpen={true}
      onClose={onClose}
      title={feature ? 'Edit Advanced Feature' : 'Add New Advanced Feature'}
      size="lg"
      footer={
