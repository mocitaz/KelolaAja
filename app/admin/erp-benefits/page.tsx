...
export default function ERPBenefitsPage() {
  ...
  const [previewBenefit, setPreviewBenefit] = useState<ERPBenefit | null>(null);
  ...
  const handleDelete = async (benefitId: number) => {
    ...
    try {
      const response = await apiFetch(`/api/v1/erp-benefits/admin/${benefitId}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        fetchBenefits();
  ...
  const filteredBenefits = benefits.filter(
    (benefit) =>
      benefit.iconName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            <SparklesIcon className="w-8 h-8 text-[#039edb]" />
            ERP Benefits
          </h1>
          <p className="text-gray-600 mt-2">Manage ERP benefits displayed on homepage</p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-[#039edb]">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Total Benefits</p>
              <p className="text-3xl font-bold text-gray-900">{statsCount.total}</p>
            </div>
            <SparklesIcon className="w-12 h-12 text-[#039edb]/20" />
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-[#71bf44]">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Active Benefits</p>
        ...
        <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-gray-400">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Inactive Benefits</p>
      ...
      <div className="bg-white rounded-xl shadow-md p-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search benefits by icon or title..."
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">
            {searchTerm ? 'No benefits found' : 'No ERP benefits yet'}
          </h3>
          <p className="text-gray-600 mb-6">
            {searchTerm
              ? 'Try adjusting your search terms'
              : 'Start by adding your first ERP benefit'}
          </p>
          {!searchTerm && (
            <button
              onClick={() => {
          ...
          {filteredBenefits.map((benefit) => (
            ...
            >
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-xs text-gray-500">Icon: {benefit.iconName}</p>
                      <p className="text-xs text-gray-500">Order: {benefit.displayOrder}</p>
                    </div>
                  </div>
                  <span
              ...
              <div className="p-4 space-y-2">
                {benefit.translations && benefit.translations.length > 0 && (
                  <div className="space-y-2">
                    {benefit.translations.slice(0, 2).map((trans, idx) => (
      ...
      {showPreview && previewBenefit && (
        <PreviewModal
          benefit={previewBenefit}
          onClose={() => {
            setShowPreview(false);
            setPreviewBenefit(null);
          }}
        />
      )}
    </div>
...
function BenefitModal({
  ...
  const [formData, setFormData] = useState({
    ...
    displayOrder: benefit?.displayOrder || 1,
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full my-8">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center rounded-t-xl">
          <h2 className="text-xl font-bold text-gray-900">
            {benefit ? 'Edit ERP Benefit' : 'Add New ERP Benefit'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
              <SparklesIcon className="w-5 h-5 text-[#039edb]" />
              Basic Information
            </h3>
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            >
              {loading ? 'Saving...' : `${benefit ? 'Update' : 'Create'} Benefit`}
            </button>
          </div>
        </form>
      </div>
...
function PreviewModal({ benefit, onClose }: { benefit: ERPBenefit; onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full">
        <div className="border-b px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-900">Benefit Preview</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
        ...
        <div className="p-6 space-y-6">
          <div className="text-center bg-gradient-to-r from-[#039edb]/5 to-[#71bf44]/5 rounded-lg p-8">
            ...
            <p className="text-sm text-gray-500">Icon: {benefit.iconName}</p>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 mb-1">Display Order</p>
              <p className="font-semibold text-gray-900">{benefit.displayOrder}</p>
            </div>
          ...
          {benefit.translations && benefit.translations.length > 0 && (
            <div className="space-y-3">
              ...
              {benefit.translations.map((trans: any, idx: number) => (
