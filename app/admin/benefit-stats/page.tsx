...
export default function BenefitStatsPage() {
  ...
  const handleDelete = async (statId: number) => {
    ...
    try {
      await apiFetch(`/api/v1/benefit-stats/admin/${statId}`, {
        method: 'DELETE',
      });
      fetchStats();
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      header: 'Icon',
      render: (stat: BenefitStat) => (
        <span className="text-xs text-gray-600">{stat.iconName || '-'}</span>
      ),
    },
    {
      header: 'Order',
      render: (stat: BenefitStat) => (
        <span className="text-xs text-gray-600">{stat.displayOrder}</span>
      ),
    },
    {
      header: 'Status',
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Manage homepage benefit statistics and counters"
        action={{
          label: 'Add Stat',
          onClick: () => {
            setEditingStat(null);
      ...
      <AdminCard compact>
        <SearchBar
          value={search}
          onChange={setSearch}
          placeholder="Search by key, value, or label..."
        />
      </AdminCard>
      ...
      <AdminTable
        ...
        data={filteredStats}
        loading={loading}
        emptyMessage="No benefit stats found. Click 'Add Stat' to create one."
      />
...
function BenefitStatModal({
  ...
  return (
    <AdminModal
      isOpen={true}
      onClose={onClose}
      title={stat ? 'Edit Benefit Stat' : 'Add New Benefit Stat'}
      size="lg"
      footer={
                ...
                <div className="space-y-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      newTranslations[idx] = { ...trans, label: e.target.value };
